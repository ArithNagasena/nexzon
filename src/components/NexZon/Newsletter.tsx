...
const Newsletter = () => {
  ...
  return (
    <section className="bg-background pb-10 sm:pb-14 lg:pb-20">
      <div className="container-page">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-deep p-6 text-primary-foreground sm:p-10 lg:p-14">
          ...
          <div className="relative grid items-center gap-6 lg:grid-cols-2 lg:gap-10">
            <div>
              ...
              <p className="mt-3 hidden max-w-md text-sm text-white/85 sm:block sm:text-base">
                Join Nexzon Insider — 40,000+ shoppers across Sri Lanka already get our best prices before
                anyone else.
              </p>

              {/* Three perks side by side on a phone, headline only — stacked full
                  cards would push the signup form off the screen. */}
              <ul className="mt-5 grid grid-cols-3 gap-2 sm:mt-6 sm:gap-3">
                {perks.map((p) => (
                  <li key={p.title} className="rounded-xl bg-white/10 p-2.5 ring-1 ring-white/15 backdrop-blur sm:p-3">
                    <p.icon className="h-4 w-4 text-white" />
                    <div className="mt-2 text-[11px] font-bold leading-tight sm:text-xs">{p.title}</div>
                    <div className="mt-1 hidden text-[11px] leading-snug text-white/70 sm:block">{p.desc}</div>
                  </li>
                ))}
              </ul>
            </div>
