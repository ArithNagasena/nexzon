...
const Categories = () => (
  <section className="bg-surface section-y">
    <div className="container-page">
      <div className="mb-5 flex items-end justify-between gap-4 sm:mb-8 lg:mb-10">
        <div>
          <span className="text-xs font-semibold uppercase tracking-wider text-primary">Shop by category</span>
          <h2 className="mt-2 font-display text-2xl font-extrabold sm:text-3xl lg:text-4xl">Browse our top categories</h2>
        </div>
        ...
        {cats.map((c, i) => (
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-3 sm:p-5 lg:p-6">
              ...
              <h3 className="mt-1 font-display text-base font-bold text-white sm:text-xl lg:text-2xl">
                {c.name}
              </h3>
              {/* The whole tile is the tap target on a phone, so the affordance
                  is only worth its line of height where there's a cursor. */}
              <span className="mt-2 hidden items-center gap-1 text-xs font-semibold text-white/95 transition-all group-hover:gap-2 sm:inline-flex sm:text-sm">
